...
| Home and Chat | Shared with Desktop | `chat` | Start from the dominant composer, choose a lead coworker, use inline `@` mentions, resume recent conversations, and create a Cloud session on first send. One optional starter appears only when recent work is empty. | Cloud Web never builds a browser-only projection model or invokes local OpenCode runtime commands. |
...
|---|---|---|---|---|---|
| Home, Chat, and composer | `chat` | Home composer, recent conversations, and focused Chat transcript | Default route prioritizes the composer, lead-coworker picker, inline `@` mentions, recent-conversation loading/empty/error states, disabled signed-out send controls, and Cloud command submission after auth. | `loading`, `empty`, `error`, `disabled`, `permission-gated`, `offline-disconnected`, `retry` | Cloud Web consumes Cloud session projections and commands only; OpenCode owns session execution and event semantics. |
...
| Coworkers, tools, and skills | `agents`, `capabilities` | Team and Tools & Skills capability catalog | Team uses All, Custom, and Built-in filters with technical provenance behind Details; Tools & Skills opens on the combined catalog and retains Tools and Skills views with MCP policy verdicts. | `loading`, `empty`, `error`, `disabled`, `permission-gated`, `retry` | Cloud Web renders policy-safe metadata only and cannot edit local custom agent files or spawn local stdio MCPs. |
...
This durable contract defines Studio parity expectations. Since Desktop and
Cloud Web are the same renderer, most rows are enforced by the renderer suite
plus the cloud HTTP/redaction suite.
