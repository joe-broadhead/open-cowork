...
| `OPEN_COWORK_CLOUD_ARTIFACT_DIRECT_UPLOAD_MODE` | Direct browser-to-store artifact upload gate. Defaults to `off`; `enabled` is accepted only when readiness proves an attested exact-size AWS S3 POST policy, durable lifecycle storage, and a live cleanup owner. The AWS credentials require `s3:GetBucketVersioning` and `s3:GetBucketCORS`. The bucket must never have had versioning enabled, must use a non-dotted virtual-host name, and its CORS rules must explicitly allow `POST` from the exact configured browser origin; path-style addressing, dotted bucket names, suspended versioning, wildcard/pattern origins, and missing CORS fail readiness closed. Leave the gate `off` to use bounded buffered upload with those providers. Provider attestations are cached only after success for 30 seconds; the scheduler's current cleanup-owner/provider lease is the authoritative cross-process gate around the local signer. Other providers remain on the bounded buffered path. |
| `OPEN_COWORK_CLOUD_ARTIFACT_DIRECT_UPLOAD_CLEANUP_BATCH_SIZE` | Maximum reservations claimed by one reconciliation pass (default and cap `100`). |
| `OPEN_COWORK_CLOUD_ARTIFACT_DIRECT_UPLOAD_CLEANUP_INTERVAL_MS` | Interval between bounded abandoned-staging cleanup passes (default `60000`). Startup runs the same reconciler before direct upload can become ready; each pass also prunes at most one batch of terminal reservation rows older than seven days. |
...
| `OPEN_COWORK_CLOUD_PROGRESS_WATCHDOG_MODE` | `worker`, `all-in-one` | Runtime progress watchdog mode: `off` (default), `observe`, or `enforce`. Missing or invalid configuration resolves to `off`; observe emits decisions but never interrupts work. |
| `OPEN_COWORK_CLOUD_PROGRESS_WATCHDOG_SUSPECT_MS` / `OPEN_COWORK_CLOUD_PROGRESS_WATCHDOG_STALLED_MS` / `OPEN_COWORK_CLOUD_PROGRESS_WATCHDOG_SWEEP_MS` | `worker`, `all-in-one` | Monotonic suspect, stalled, and single-loop sweep intervals. Defaults are `120000`, `300000`, and `5000`; safe minima are `1000`, `2000`, and `250`, and suspect must remain below stalled. Smaller or inverted values fail closed to `off`. Polls, health checks, duplicate events, and bare lease renewals do not reset either threshold. |
| `OPEN_COWORK_CLOUD_PROGRESS_WATCHDOG_MAX_ENTRIES` / `OPEN_COWORK_CLOUD_PROGRESS_WATCHDOG_MAX_SNAPSHOT_ENTRIES` | `worker`, `all-in-one` | Hard bounds for in-memory tracked executions and privacy-safe `/progressz` samples. Snapshots contain only state, age, bounded source, mode, and generation—never session/run/tenant IDs, prompts, output, paths, or lease tokens. |
| `OPEN_COWORK_CLOUD_PROGRESS_WATCHDOG_OBSERVE_EVIDENCE_REF` / `OPEN_COWORK_CLOUD_PROGRESS_WATCHDOG_OPERATOR_OWNER` / `OPEN_COWORK_CLOUD_PROGRESS_WATCHDOG_ROLLBACK_MODE` | `worker`, `all-in-one` | Mandatory enforcement gate: a canary observe-evidence reference, named operator owner, and rollback mode (`off` or `observe`). If any field is missing, `enforce` resolves to `off`. |
...
| `OPEN_COWORK_CLOUD_ISOLATION_OPENCODE_BIN` | `worker`, `all-in-one` | Absolute in-image path to the OpenCode executable. Defaults to the direct production dependency binary in the standard `open-cowork-cloud` image (`/app/node_modules/.bin/opencode`). Relative and parent-traversing paths fail closed. |
...
Watchdog decisions and recovery outcomes emit only the closed metric dimensions
`watchdog_state` (`suspect` or `stalled`) and `watchdog_outcome`. The worker also
writes an idempotent system audit event to the session tenant's resolved org; its
metadata is limited to mode, state, outcome, and source, with no tenant, session,
run, lease, prompt, output, or path identifiers.
