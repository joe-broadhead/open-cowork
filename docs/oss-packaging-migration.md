...
| Open Cowork Cloud | Supported for self-host beta and private hosted beta | cloud role code under `apps/desktop/src/main/cloud`; the browser UI is the unified renderer (`packages/app/src`) served by the cloud at `GET /` | `open-cowork-cloud` OCI image and Helm/Compose assets |
