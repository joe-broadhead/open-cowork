...
| Gateway cloud binding | env/deployment secrets | `OPEN_COWORK_CLOUD_BASE_URL`, `OPEN_COWORK_GATEWAY_SERVICE_TOKEN`, `OPEN_COWORK_GATEWAY_CLOUD_REQUEST_TIMEOUT_MS`, and loopback-only insecure HTTP policy. File-backed gateway cloud connection fields are rejected. |
...
| Runtime profiles and policy packs | `cloud.profiles`, `cloud.runtime`, `cloud-config.ts`, `cloud-runtime-capability-policy.ts`, `packages/runtime-host/src/runtime-config-builder.ts` | Cloud profiles force app-managed runtime config by default and compile tool/MCP allowlists into the final OpenCode runtime config. Explicit empty lists deny all; unknown ids fail closed. Machine config and arbitrary local stdio MCPs remain disabled without explicit policy. |
...
| BYOK provider validation/injection | `byok-secret-store.ts`, `packages/runtime-host/src/runtime-config-builder.ts`, `opencode-runtime-adapter.ts`, `cloud-config.ts` | Provider keys enter OpenCode runtime config as provider options, never process env, logs, renderer state, diagnostics, or cache. |
