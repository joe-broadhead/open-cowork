...
- Workflow webhook credentials are never accepted by the MCP, generated by its
  transcript-persisted create operation, or returned in tool results. Users
  provision the credential through the one-time Playbooks secret mutation after
  saving a webhook-enabled workflow.
