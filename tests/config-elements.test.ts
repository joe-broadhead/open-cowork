...
test('open core ships with built-in tools, skills, mcps, and agents configured by default', () => {
  ...
  assert.equal(tools.map((tool) => tool.id).join(','), 'clock,charts,skills,agents,workflows')
  ...
  assert.equal(mcps.map((mcp) => mcp.name).join(','), 'clock,charts,skills,agents,workflows')
