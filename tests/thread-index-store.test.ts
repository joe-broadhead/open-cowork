...
test('thread index rejects missing, older, future, and shape-drifted stores before WAL or schema writes', () => {
  const root = mkdtempSync(join(tmpdir(), 'open-cowork-thread-index-noncurrent-'))
  try {
    for (const scenario of ['missing', 'older', 'future', 'shape'] as const) {
      const dbPath = join(root, `${scenario}.sqlite`)
      const db = new DatabaseSync(dbPath)
      db.exec(`
        create table preserved_user_metadata (value text not null);
        insert into preserved_user_metadata (value) values ('keep-me');
      `)
      if (scenario !== 'missing') {
        db.exec('create table thread_index_meta (key text primary key, value text not null)')
        const declaredVersion = scenario === 'older'
          ? THREAD_INDEX_SCHEMA_VERSION - 1
          : scenario === 'future' ? THREAD_INDEX_SCHEMA_VERSION + 1 : THREAD_INDEX_SCHEMA_VERSION
        db.prepare("insert into thread_index_meta (key, value) values ('schema_version', ?)")
      ...
      assert.throws(
        () => new ThreadIndexStore(dbPath),
        /Back up the thread-index database.*left untouched/,
      )
      assert.deepEqual(readFileSync(dbPath), before, `${scenario} store bytes must be preserved`)
    }
  } finally {
    rmSync(root, { recursive: true, force: true })
  }
