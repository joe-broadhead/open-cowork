...
describe('buildModelInfoSnapshot', () => {
  ...
  it('overlays provider pricing on top of fallbacks', () => {
    ...
    const snap = buildModelInfoSnapshot([
      {
        models: {
          'acme/alpha': { cost: { input: 3, output: 15, cache_read: 0.5 }, limit: { context: 200_000 } },
          'acme/beta': { cost: { input: 5, output: 20 }, limit: { context: 128_000 } },
        },
      },
    ], fallbacks)
    assert.deepEqual(snap.pricing['acme/alpha'], { inputPer1M: 3, outputPer1M: 15, cachePer1M: 0.5 })
