...
test('dist gateway product mode helpers keep cloud and standalone apps separate', () => {
  ...
  assert.throws(() => resolveStandaloneGatewayProductMode('cloud_channel', undefined), /apps\/channel-gateway/)
...
test('source gateway product mode helpers reject cross-product configuration', () => {
  ...
  assert.throws(() => sourceGatewayModes.resolveStandaloneGatewayProductMode('cloud_channel', undefined), /apps\/channel-gateway/)
})
