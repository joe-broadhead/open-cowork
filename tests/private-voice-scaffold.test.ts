...
test('private voice: IPC and preload channels are scaffolded', () => {
  ...
  assert.match(conversation, /HOST_AUTO_FINALIZE/)
  assert.match(conversation, /JOE-1107/)

  const vad = readFileSync(join(root, 'apps/desktop/src/main/voice-vad.ts'), 'utf8')
  assert.match(vad, /tickVad/)
  assert.match(vad, /isBargeInSpeech/)
  assert.match(vad, /JOE-1104/)
  assert.match(host, /continuousVad/)
  assert.match(host, /tickVad/)
  assert.match(host, /startBargeInMonitor/)

  const sharedVoice = readFileSync(join(root, 'packages/shared/src/voice.ts'), 'utf8')
  assert.match(sharedVoice, /VoiceVadEvent/)
  assert.match(sharedVoice, /continuousVad/)
  assert.match(sharedVoice, /barge_in/)
  assert.match(sharedVoice, /VoiceAssetStatusSnapshot/)

  const assets = readFileSync(join(root, 'apps/desktop/src/main/voice-assets.ts'), 'utf8')
  assert.match(assets, /probeVoiceAssets/)
  assert.match(assets, /ensureVoiceSttAsset/)
  assert.match(assets, /JOE-1109/)
  assert.match(assets, /fail-closed|local_only/i)
  assert.match(handlers, /voice:assets:status/)
