...
test('buildOpenCoworkAgentConfig exposes the generic OpenCode agent set', () => {
  ...
  assert.equal(agents['chief-of-staff'].mode, 'primary')
  assert.match(agents['chief-of-staff'].prompt, /Chief-of-Staff planner/i)
  assert.match(agents['chief-of-staff'].prompt, /CoordinationTask contract/i)
  assert.equal(agents['chief-of-staff'].permission.todowrite, 'allow')
  assert.equal(agents['chief-of-staff'].permission.question, 'allow')
  assert.equal(agents['chief-of-staff'].permission.task.explore, 'allow')
  assert.equal(agents['chief-of-staff'].permission.task.general, undefined)
  assert.equal(agents['chief-of-staff'].permission.write, 'deny')
...
test('built-in agent details expose the native OpenCode agent set plus configured built-in agents', () => withIsolatedSettings('agent-details-', () => {
  ...
  assert.deepEqual(names, ['build', 'plan', 'general', 'explore', 'chief-of-staff', 'autoresearch', 'executive-assistant', 'charts', 'skill-builder', 'agent-builder', 'workflow-designer', 'research'])
  ...
  const chiefOfStaff = builtins.find((agent) => agent.name === 'chief-of-staff')
  assert.equal(chiefOfStaff?.hidden, false)
  assert.equal(chiefOfStaff?.surface, 'chat')
  assert.equal(chiefOfStaff?.label, 'Cleo')
  assert.deepEqual(chiefOfStaff?.nativeToolIds, ['read', 'grep', 'glob', 'list', 'websearch', 'webfetch', 'todowrite', 'question'])
