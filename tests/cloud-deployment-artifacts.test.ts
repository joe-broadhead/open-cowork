...
test('deployment validation and smoke scripts cover compose, helm, cloud, and gateway checks', () => {
  ...
  assert.match(packageJson, /"deploy:gateway:smoke": "corepack pnpm build:gateway && node scripts\/gateway-cloud-smoke\.mjs"/)
  assert.match(packageJson, /"deploy:continuation:smoke": "corepack pnpm build:gateway && corepack pnpm build:shared && node --no-warnings --experimental-strip-types scripts\/cloud-continuation-smoke\.mjs"/)
  assert.match(packageJson, /"ops:validate": "node --no-warnings --experimental-strip-types scripts\/check-opencode-compatibility\.ts && node scripts\/validate-ops-readiness\.mjs && node scripts\/validate-release-gates\.mjs"/)
  assert.match(packageJson, /"release:gates:validate": "node scripts\/validate-release-gates\.mjs"/)
