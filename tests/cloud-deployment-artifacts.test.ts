...
test('CI enforces cloud portability, concurrency, and deployment gates', () => {
  ...
  assert.match(workflow, /OPEN_COWORK_GATEWAY_ADMIN_TOKEN=ci-gateway-admin-token docker compose -f docker-compose\.cloud-gateway\.yml config --quiet/)
  ...
  assert.match(workflow, /OPEN_COWORK_GATEWAY_ADMIN_TOKEN=ci-gateway-admin-token OPEN_COWORK_GATEWAY_ENABLE_FAKE_PROVIDER=true OPEN_COWORK_GATEWAY_ALLOW_PUBLIC_FAKE_PROVIDER=true OPEN_COWORK_GATEWAY_SMOKE_URL=http:\/\/127\.0\.0\.1:8790\/ready bash scripts\/ci-cloud-compose-smoke\.sh docker-compose\.cloud-gateway\.yml/)
