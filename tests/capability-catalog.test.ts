...
test('built-in capabilities expose discoverable metadata without source parsing', async () => {
  ...
  assert.equal(tools.some((tool) => tool.id === 'time-keep'), true)
  ...
  assert.equal(skills.some((skill) => skill.name === 'time-keep'), true)
  ...
  assert.deepEqual(skills.find((skill) => skill.name === 'time-keep')?.toolIds, ['time-keep'])
