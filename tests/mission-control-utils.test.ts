...
describe('computeLaneProgress', () => {
  ...
  it('scales running tasks against the group max', () => {
    ...
    const task = makeTask({ status: 'running', startedAt: '2026-01-01T00:00:00Z' })
    const progress = computeLaneProgress(task, 4_000, now)
    assert.equal(progress, 0.5)
  })

  it('uses the supplied clock so live lanes can advance between task-store updates', () => {
    const task = makeTask({ status: 'running', startedAt: '2026-01-01T00:00:00Z' })
    const early = computeLaneProgress(task, 10_000, Date.parse('2026-01-01T00:00:01Z'))
    const later = computeLaneProgress(task, 10_000, Date.parse('2026-01-01T00:00:04Z'))
    assert.ok(later > early, `expected later progress ${later} to exceed early progress ${early}`)
  })

  it('clamps running tasks to a minimum visible fill so the lane is not empty', () => {
    const task = makeTask({ status: 'running', startedAt: '2026-01-01T00:00:00Z' })
    const progress = computeLaneProgress(task, 60_000, Date.parse('2026-01-01T00:00:00.005Z'))
    assert.equal(progress, 0.04)
  })
})
