...
| Home | Chat | Team |
|:---:|:---:|:---:|
| ![Home composer with lead assignment and a first-conversation starter](docs/assets/auto/home.png) | ![New Chat composer with Review collapsed by default](docs/assets/auto/chat.png) | ![Team page with built-in coworkers and provenance filters](docs/assets/auto/team.png) |
| Composer-first landing surface with recent work. | OpenCode execution in a focused desktop transcript. | Built-in and custom coworkers with honest provenance. |

| Tools & Skills | Playbooks | Projects |
|:---:|:---:|:---:|
| ![Tools & Skills catalog with tools, skills, and MCP servers](docs/assets/auto/tools-skills.png) | ![Playbooks first-run state with one setup path](docs/assets/auto/playbooks.png) | ![Projects board with workspace-backed coordination](docs/assets/auto/projects.png) |
| Tools, skills, and MCPs with partial-result handling. | Repeatable work created from setup chats and run through OpenCode. | Workspace-backed coordination without synthetic project state. |
...
- Playbooks created from Workflow Designer setup chats, with manual, scheduled, and webhook runs.
...
4. Connect a provider: OpenRouter API key, OpenAI/Codex via ChatGPT Plus/Pro or API key, or GitHub Copilot via OpenCode's native Copilot login.
5. Start a thread:
...
7. Use **Playbooks** for repeatable work that should be saved from a Workflow Designer setup chat and run manually, on a schedule, or from a webhook.
...
The GitHub Pages publish URL is set explicitly in the docs workflow (the `OPEN_COWORK_SITE_URL`, `OPEN_COWORK_REPO_URL`, and `OPEN_COWORK_REPO_NAME` env values in `.github/workflows/docs.yml`), so renaming the repository means updating those literals.
