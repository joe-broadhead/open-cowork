...
function scheduleRuntimeRefresh() {
  const scheduler = runtimeRefreshScheduler || (() => {
    // The desktop injects a real scheduler (rebootRuntime) via
    // configureAgentToolBridge; hosts that don't (e.g. the cloud server) treat a
    // runtime refresh as a no-op rather than reaching into a desktop entrypoint.
    log('agent-tool-bridge', 'Runtime refresh requested but no scheduler is configured; ignoring.')
  })
