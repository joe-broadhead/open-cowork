...
// Injected by the desktop (diagnostics-export stays desktop-side); the cloud leaves
// it null and the diagnostics.export action reports it unavailable.
let diagnosticsBundleBuilder: (() => string) | null = null
...
async function executeDefaultAction(actionId: SemanticUiActionId, input: Record<string, unknown>) {
  ...
  if (!diagnosticsBundleBuilder) {
    return createSemanticUiActionResult({
      capturedAt: new Date().toISOString(),
      actionId,
      ok: false,
      errorCode: 'semantic-ui-action-unavailable',
      message: 'Diagnostics export is not available on this host.',
    })
  }
...
export function configureSemanticUiBridge(options: {
  ...
  diagnosticsBundleBuilder?: (() => string) | null
} = {}) {
  ...
  diagnosticsBundleBuilder = options.diagnosticsBundleBuilder || null
}
