...
function unpackedResourcePath(value: string) {
  ...
  return value.replace(`${resolve(((process as { resourcesPath?: string }).resourcesPath ?? process.cwd()), 'app.asar')}`, resolve(((process as { resourcesPath?: string }).resourcesPath ?? process.cwd()), 'app.asar.unpacked'))
}
...
function resolveBundledNodeModuleDir(moduleName: string): string | null {
  ...
  if (getAppPathHost()?.isPackaged) {
    const unpacked = join(((process as { resourcesPath?: string }).resourcesPath ?? process.cwd()), 'app.asar.unpacked', 'node_modules', moduleName)
...
function resolveDevelopmentPackageJsonPath(moduleName: string): string | null {
  ...
  // node_modules/<module>/package.json. Packages whose `exports` block direct
  // package.json access (e.g. @opencode-ai/sdk) can't be located via
  // require.resolve, so this resolves them by filesystem layout. Walking
  // ancestors keeps this correct regardless of how deep the module sits inside
  // the package (src/ vs src/main/, source vs dist).
  const moduleParts = moduleName.split('/').filter(Boolean)
  let dir = currentModuleDir
  while (true) {
    const candidate = resolve(dir, 'node_modules', ...moduleParts, 'package.json')
    if (existsSync(candidate)) return candidate
    const parent = dirname(dir)
    if (parent === dir) return null
    dir = parent
