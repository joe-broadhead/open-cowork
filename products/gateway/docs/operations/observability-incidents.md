...
`/observability` returns `trace`, `slo`, and `support` objects alongside metrics, alerts, supervisors, and environments. `/metrics` exposes the local Prometheus registry, including `gateway_live_sse_active`, `gateway_live_sse_rejected_total`, `gateway_live_sse_timeouts_total`, `gateway_live_sse_slow_consumers_total`, `gateway_live_upstream_frames_rejected_total`, `gateway_live_sse_replay_snapshots`, `gateway_live_sse_replay_bytes`, `gateway_live_sse_replay_dropped_total`, and `gateway_progress_watchdog_outcomes_total`. Replay-drop reasons are the bounded labels `snapshot_limit`, `payload_limit`, and `total_bytes_limit`; watchdog outcomes are the bounded labels `suspect`, `stalled`, `enforced`, `fenced_stale`, `recovered`, and `failed`. `/progressz` is the separate privacy-safe watchdog operator surface: it contains only mode/status, aggregate state counts, bounded age/source/generation samples, and truncation. Trace IDs are deterministic correlation IDs such as `trace_task_*` and `trace_run_*`; raw channel targets, authenticated principal IDs, malformed upstream frames, session IDs, run IDs, lease owners, local private paths, and token-like evidence refs are hashed, omitted, or redacted from metric labels and `/progressz`.

The Gateway watchdog advances only on the closed OpenCode progress vocabulary. It fingerprints bounded semantic samples for assistant phases, tool calls/progress, shell transitions, and output deltas so exact stream replays do not refresh a run while distinct calls and evolving results do. Every sweep also reconciles each tracked identity with the durable run record; a missed OpenCode terminal event therefore cannot leave a passed, failed, or replaced run reported as stalled. Failed or transiently fenced recovery remains retryable with capped backoff, while a confirmed recovery removes that exact generation from `/progressz` immediately.
