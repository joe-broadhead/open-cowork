...
Durable Gateway is a **single-writer** control plane per state directory. Helm charts refuse `replicaCount > 1` unless `gateway.experimentalDistributedOwnership=true` (lab-only; not multi-AZ HA). JOE-996 closed open migrate hazards (H1 channel-sync coordination SQLite, H3/H4/H8 operational sidecar, H13 notification send leases); proving registry may report `status=ready`. Process-local debounce maps remain single-daemon. See [Multi-Daemon Scaling](../concepts/multi-daemon-scaling.md) and [Distributed ownership design](../concepts/distributed-ownership-design.md). Do **not** claim multi-AZ HA without an explicit product decision (claims script / `marketingForbiddenClaims`).
