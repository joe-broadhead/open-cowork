...
| `OPENCODE_GATEWAY_PROGRESS_WATCHDOG_MODE` | `off` (default), `observe`, or `enforce`. Missing and invalid values fail closed to `off`. Off allocates no watchdog timer or event consumer; observe records decisions without changing run/OpenCode state; enforce applies only lease-fenced stalled-run recovery. `opencode-gateway install` copies only the documented printable watchdog variables into the managed service definition; rerun install after changing them |
| `OPENCODE_GATEWAY_PROGRESS_WATCHDOG_SUSPECT_MS` / `OPENCODE_GATEWAY_PROGRESS_WATCHDOG_STALLED_MS` | Bounded thresholds (`1s`–`24h` suspect, `2s`–`7d` stalled; defaults `2m` and `5m`). Stalled must be greater than suspect or the watchdog stays off as invalid |
| `OPENCODE_GATEWAY_PROGRESS_WATCHDOG_SWEEP_MS` | Bounded evaluation cadence (`250ms`–`1h`; default `10s`) |
| `OPENCODE_GATEWAY_PROGRESS_WATCHDOG_MAX_ENTRIES` / `OPENCODE_GATEWAY_PROGRESS_WATCHDOG_MAX_SNAPSHOT_ENTRIES` | Bounded in-memory run tracking (`1`–`10000`, default `1000`) and privacy-safe `/progressz` sample count (`0`–`100`, default `20`) |
| `OPENCODE_GATEWAY_PROGRESS_WATCHDOG_OBSERVE_EVIDENCE_REF` / `OPENCODE_GATEWAY_PROGRESS_WATCHDOG_OPERATOR_OWNER` / `OPENCODE_GATEWAY_PROGRESS_WATCHDOG_ROLLBACK_MODE` | Required enforcement gate. `enforce` stays off unless observe evidence and an operator owner are nonempty and rollback is explicitly `observe` or `off`. Values are never emitted in logs, labels, or `/progressz` |
