...
export const CLOUD_WEB_ROUTE_API_MATRIX: CloudWebRouteApiMatrixEntry[] = [
  {
    ...
    states: {
      ...
      empty: 'No chats loaded; selected-chat objectives explain when no projected objective exists.',
    ...
    pagination: 'Browser page size is bounded by CLOUD_WEB_THREAD_PAGE_SIZE. Cloud Web consumes /api/sessions cursor pages with Load more, preserves loaded pages across workspace SSE refreshes, and only shows total estimates when the backend returns one. Objectives are selected-chat scoped until a Cloud objective index exists.',
  ...
  },
  {
    ...
    disabledBehavior: 'The user-facing Channels route is read-only and focuses on reach, delivery status, and linked chats.',
    pagination: 'Channel agents and bindings load the first 100 rows; delivery status loads the first 50 rows with member-facing bounded-list copy.',
    paginationContract: paginationContract('bounded-page', 'deferred', 100),
  ...
  },
  {
    ...
    pagination: 'Selected-session artifact lists use the cloud artifact API; selected-chat history is explicit and cross-session artifact browsing is deferred.',
    paginationContract: paginationContract('bounded-page', 'deferred', 100),
