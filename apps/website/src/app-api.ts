  ...
  return {
    ...
    admin: {
      ...
      byok: {
        ...
        save: (providerId, input) => request(`/api/byok/${encodeURIComponent(providerId)}`, { method: 'POST', body: input }),
        validate: (providerId) => request(`/api/byok/${encodeURIComponent(providerId)}/validate`, { method: 'POST' }),
        disable: (providerId) => request(`/api/byok/${encodeURIComponent(providerId)}`, { method: 'DELETE' }),
      },
      apiTokens: {
        list: () => request(endpoint('apiTokens', '/api/api-tokens?limit=100')),
        create: (input) => request('/api/api-tokens', { method: 'POST', body: input }),
        revoke: (tokenId) => request(`/api/api-tokens/${encodeURIComponent(tokenId)}`, { method: 'DELETE' }),
      },
      channels: {
        agents: () => request(endpoint('channelAgents', '/api/channels/agents?limit=100')),
        createAgent: (input) => request('/api/channels/agents', { method: 'POST', body: input }),
        bindings: () => request(endpoint('channelBindings', '/api/channels/bindings?limit=100')),
        createBinding: (input) => request('/api/channels/bindings', { method: 'POST', body: input }),
        deliveries: () => request(endpoint('channelDeliveries', '/api/channels/deliveries?limit=50')),
      ...
      billing: {
        ...
        checkout: (input) => request('/api/billing/checkout', { method: 'POST', body: input }),
        portal: () => request('/api/billing/portal', { method: 'POST', body: {} }),
      },
      usage: {
