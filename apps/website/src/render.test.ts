...
void test('cloud website renders Studio and admin shell surfaces', () => {
  ...
  assert.match(html, /data-route-panel="approvals"/)
  assert.match(html, /data-route-panel="channels"/)
  assert.match(html, /data-route-panel="settings"/)
  ...
  assert.match(html, /Assign to: profile default/)
  assert.match(html, /id="composer-agent-chips"/)
  assert.match(html, /id="cloud-launchpad-home"/)
  ...
  assert.match(html, /Fresh artifacts/)
  ...
  assert.match(html, /data-parity-route="approvals"/)
  assert.match(html, /data-parity-route="settings"/)
  ...
  assert.match(html, /class="cloud-settings-surface"/)
  assert.match(html, /id="cloud-settings-profile"/)
  assert.match(html, /id="cloud-settings-appearance"/)
  assert.match(html, /id="cloud-settings-notifications"/)
...
void test('cloud website app shell exposes typed route metadata', () => {
  ...
  assert.equal(findCloudWebRoute('settings')?.requiresAdmin, false)
  assert.equal(findCloudWebRoute('settings')?.surface, 'workbench')
...
void test('cloud website bootstrap exposes typed client endpoint metadata', () => {
  ...
  assert.equal(CLOUD_WEB_CLIENT_ENDPOINTS.find((endpoint) => endpoint.id === 'coordinationWatches')?.path, '/api/coordination/watches?limit=500')
  assert.equal(CLOUD_WEB_CLIENT_ENDPOINTS.find((endpoint) => endpoint.id === 'coordinationWatchCreate')?.method, 'POST')
  assert.equal(CLOUD_WEB_CLIENT_ENDPOINTS.find((endpoint) => endpoint.id === 'coordinationWatchPause')?.path, '/api/coordination/watches/:watchId/pause')
  assert.equal(CLOUD_WEB_CLIENT_ENDPOINTS.find((endpoint) => endpoint.id === 'coordinationWatchDelete')?.method, 'DELETE')
