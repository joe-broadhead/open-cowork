...
describe('appearance theme defaults', () => {
  it('defaults first-run appearance to Mercury with the Azure accent', () => {
    ...
    expect(document.documentElement.style.getPropertyValue('--accent-action-foreground')).toBe('#ffffff')
    ...
    expect(document.documentElement.style.getPropertyValue('--accent-action-fill')).toBe('linear-gradient(rgba(0,0,0,0.17),rgba(0,0,0,0.17)), var(--accent-gradient)')
  })
})
