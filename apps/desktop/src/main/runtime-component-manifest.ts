...
import { resolveBundledMcpScriptPath } from './runtime-mcp.ts'
import { recordRuntimeComponentVerification } from './runtime-status.ts'
...
export function resolveRuntimeComponentManifestPath(input: RuntimeComponentManifestBuildInput = {}) {
  ...
  if (isPackaged(input)) {
    return join(input.resourcesPath || process.resourcesPath, RUNTIME_COMPONENT_MANIFEST_FILE)
  }
