...
const MAX_CHART_SPEC_BYTES = 256 * 1024
const MAX_CHART_ARRAY_ITEMS = 20_000
const MAX_CHART_NODE_COUNT = 10_000
const MAX_CHART_DEPTH = 32
...
function validateChartSpec(value: unknown) {
  const serialized = JSON.stringify(value)
  const serializedBytes = Buffer.byteLength(serialized, 'utf8')
  if (serializedBytes > MAX_CHART_SPEC_BYTES) {
    throw createBlockedChartRenderError(`spec exceeds ${MAX_CHART_SPEC_BYTES} bytes`)
  }
  ...
  const visit = (entry: unknown, depth: number) => {
    if (depth > MAX_CHART_DEPTH) {
      throw createBlockedChartRenderError(`spec exceeds maximum depth of ${MAX_CHART_DEPTH}`)
    }

    if (Array.isArray(entry)) {
      arrayItems += entry.length
      if (arrayItems > MAX_CHART_ARRAY_ITEMS) {
        throw createBlockedChartRenderError(`spec exceeds ${MAX_CHART_ARRAY_ITEMS} total array items`)
      }
    ...
    nodeCount += 1
    if (nodeCount > MAX_CHART_NODE_COUNT) {
      throw createBlockedChartRenderError(`spec exceeds ${MAX_CHART_NODE_COUNT} object nodes`)
    }

    const record = entry as Record<string, unknown>
    const mark = record.mark
    if (typeof mark === 'string' && mark.toLowerCase() === 'image') {
      throw createBlockedChartRenderError('image marks are not allowed')
    }
    if (mark && typeof mark === 'object' && typeof (mark as Record<string, unknown>).type === 'string'
      && String((mark as Record<string, unknown>).type).toLowerCase() === 'image') {
      throw createBlockedChartRenderError('image marks are not allowed')
    }

    for (const [key, child] of Object.entries(record)) {
      if (BLOCKED_RESOURCE_KEYS.has(key) && typeof child === 'string' && child.trim().length > 0) {
        throw createBlockedChartResourceError(`${key}="${child}" is not allowed`)
      }
      visit(child, depth + 1)
    }
  }
