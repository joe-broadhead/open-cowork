...
test('packaged app launches cleanly and preserves sessions across relaunch', async () => {
  const executablePath = process.env.OPEN_COWORK_PACKAGED_EXECUTABLE?.trim()
  assert.ok(
    executablePath,
  ...
  try {
    firstLaunch = await launchSmokeSession(paths, { executablePath })
    await waitForAppShell(firstLaunch.page, 30_000)
    ...
    secondLaunch = await launchSmokeSession(paths, { executablePath })
    await waitForAppShell(secondLaunch.page, 30_000)
